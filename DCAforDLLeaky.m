...
for i=1:N
    ...
    else %(Xval((j-1)*d+1:j*d))'*(X(i,:))'>-0.1;
       v((j-1)*d+1:j*d)=v((j-1)*d+1:j*d)+X(i,:)*alpha;
    end;
    % some approximation of subgradient   
    if (Xval(n*d+(j-1)*d+1:n*d+j*d))'*(X(i,:))'>=0
        v(n*d+(j-1)*d+1:n*d+j*d)=v(n*d+(j-1)*d+1:n*d+j*d)+X(i,:);
    else % (Xval(n*d+(j-1)*d+1:n*d+j*d))'*(X(i,:))'>-0.1;
       v(n*d+(j-1)*d+1:n*d+j*d)=v(n*d+(j-1)*d+1:n*d+j*d)+X(i,:)*alpha;
    end;   
        end;
end;
...
A=zeros(2*N,2*n*d+1+2*n*N);% Top 2N rows
...
A=[V1 V2 -ones(N,1) Z0 Z2;
   V1 V2 -ones(N,1) Z2 Z0];
...
%     a         b             z            z+         z-
B=[Z         zeros(N*n,d*n) zeros(N*n,1) -eye(n*N) zeros(n*N);
  zeros(N*n,d*n) Z          zeros(N*n,1) zeros(n*N) -eye(n*N);
  Z*alpha         zeros(N*n,d*n) zeros(N*n,1) -eye(n*N) zeros(n*N);
  zeros(N*n,d*n) Z*alpha          zeros(N*n,1) zeros(n*N) -eye(n*N)];

%LHS
A=[A;B];
%Cost vector
f=zeros(1,2*n*d+1+2*N*n);
f(2*d*n+1)=1;
%RHS
b=[-Y;Y;zeros(2*2*n*N,1)];
%LB
%LB=[-inf*ones(2*n*d,1);zeros(1+2*n*N,1)];
LB=[-inf*ones(2*n*d+1+2*n*N,1)];
UB=inf*(ones(2*n*d+1+2*N*n,1));
%[Xval,val]=linprog(f,A,b,[],[],LB,UB);
[Xval,val]=linprog(f,A,b);
...
for i=1:N
    ...
    Ya2(i)=0;
    for j=1:n
    Ya1(i)=Ya1(i)+max(alpha*(X(i,:)*Xval((j-1)*d+1:j*d)),X(i,:)*Xval((j-1)*d+1:j*d));
    Ya2(i)=Ya2(i)+max(alpha*(X(i,:)*Xval(n*d+(j-1)*d+1:n*d+j*d)),X(i,:)*Xval(n*d+(j-1)*d+1:n*d+j*d));
    end;
    Ya(i)=Ya1(i)-Ya2(i);
end;
error=(Ya'-Y);
Iter(ll)=max(abs(error))
end;